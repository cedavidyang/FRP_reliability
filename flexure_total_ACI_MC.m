...
for iSmp = 1:nSim
    ...
    for iC = 1:MAX_INITIAL_GUESS
        cIni = cIniArray(iC);
        [c(iSmp), fval, exitflag] =...
            fsolve( @(c) force_balance(alpha1, beta1(iSmp), EsMPA(iSmp), EsCmpMPA(iSmp),...
            EfrpMPA(iSmp),eFrpIni, eConcUlti, eFrpCritical(iSmp),...
            bBeamMM(iSmp), bFlangeMM(iSmp), tFlangeMM(iSmp),...
            dBeamMM(iSmp), dCmpMM(iSmp), hBeamMM(iSmp), c,...
            fsMPA(iSmp), fsCmpMPA(iSmp), fcMPA(iSmp),...
...
for iCol = 1:nCol
    ...
    Ac( cr>tf & bf~=b ) = b( cr>tf & bf~=b ) .* cr( cr>tf & bf~=b ) + ...
        ( bf( cr>tf & bf~=b )-b( cr>tf & bf~=b )) .*tf( cr>tf & bf~=b ); % Type II T section
