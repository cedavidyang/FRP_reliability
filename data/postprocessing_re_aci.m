...
switch frp_form
    case 1
        beam = 'shear';
        struct_s = load(strcat('data_', code, '_', beam, '+side_', fc, '_', num2str(fccov), '.mat'));
        struct_u = load(strcat('data_', code, '_', beam, '+U_', fc, '_', num2str(fccov), '.mat'));
        struct_w = load(strcat('data_', code, '_', beam', '+W_', fc, '_', num2str(fccov), '.mat'));
    ...
    case 2
        beam = 'flexure';
        struct_ic = load(strcat('data_', code, '_', beam, '+IC_', fc, '_', num2str(fccov), '.mat'));
        struct_rup = load(strcat('data_', code, '_', beam, '+rupture_', fc, '_', num2str(fccov), '.mat'));
        life_ic = 50*log(normcdf(beta_T_50))./log(normcdf(struct_ic.mean_RE)); life_ic(life_ic>MAX_YR) = MAX_YR;
        life_rup = 50*log(normcdf(beta_T_50))./log(normcdf(struct_rup.mean_RE)); life_rup(life_rup>MAX_YR) = MAX_YR;
        ...
        hic1 = plot(FACTOR_FRP, struct_ic.mean_RE, 'ko-', 'LineWidth', 1, 'MarkerFace', 'k', 'MarkerSize', 4);
        hrup1 = plot(FACTOR_FRP, struct_rup.mean_RE, 'k^-', 'LineWidth', 1, 'MarkerFace', 'k', 'MarkerSize', 4);
        ...
        hic2 = plot(FACTOR_FRP, life_ic, 'ko-', 'LineWidth', 1, 'MarkerFace', 'k', 'MarkerSize', 4);
        hrup2 = plot(FACTOR_FRP, life_rup, 'k^-', 'LineWidth', 1, 'MarkerFace', 'k', 'MarkerSize', 4);
